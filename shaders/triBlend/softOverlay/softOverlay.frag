...
void main() {
	...
	if (textureRGBA_1.a != 0) {
        ...
        if (textureRGBA_2.a != 0) {
		    ...
		    vec3 maskedMapPrePre = BlendOverlay(maskedMapPre, textureRGBA_2.rgb);
		    maskedMapPrePre = BlendAverage(maskedMapPrePre, textureRGBA_2.rgb);


            vec4 color = vec4(max(maskedMapPrePre, textureRGBA_1.rgb), textureRGBA_2.a);
